...
import { auth, fireStoreCollectionReference, firebaseStorage } from '../../FirebaseInitialisation';
...
export default function EditUserDetails() {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                setSignedInUserId(user.uid); // if the user is logged in, then we can get the user uid like this.
            }
        });
    ...
    async function handleOnSubmitEditUserDetails(e) {
        ...
        try {
            ...
            // make sure that both profile picture and cover picture run the code if they are not null. If no changes are done, the values are null. 
            if (profilePicture) {
        ...
        <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
            <form className="space-y-6" onSubmit={handleOnSubmitEditUserDetails}>
                ...
                {updateSuccessMessage && (
                    <div className="text-green-500 text-sm">{updateSuccessMessage}</div>
                )}
